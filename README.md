...
By default `.env.example` selects the `dev` profile, wires the app to the local Postgres container, and reads placeholder values from `.env.example`. If you need a prod-like local run through Compose, override `SPRING_PROFILES_ACTIVE=prod` in your `.env` before starting the stack.
...
This expects a reachable Postgres instance and uses `TODO_DB_*` values from your shell environment or `.env`.
